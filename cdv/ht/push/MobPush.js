...
function MobPush()
{
    ...
    var AndroidAPICaller = function ()
    {
        ...
        this.callMethod = function (request)
        {
            ...
            //java接口
            window.JSInterfaceMobPush.jsCallback(request.seqId.toString(), request.method, ObjectToJsonString(request.params), "$mobpush.callback");
        };
    ...
    this.initMobPushJS = function (platform)
    {
        switch (platform)
        {
            case 1:
                jsLog = {
                log: function(msg) {
                    window.JSInterfaceMobPush.jsLog(msg);
                }
                };
                if(isDebug) {
